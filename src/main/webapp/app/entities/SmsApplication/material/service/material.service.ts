...
type RestOf<T extends IMaterial | NewMaterial> = Omit<T, 'updateDate' | 'createDate'> & {
...
export class MaterialService {
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/materials', 'smsapplication');
  ...
  protected convertDateFromClient<T extends IMaterial | NewMaterial | PartialUpdateMaterial>(material: T): RestOf<T> {
    return {
      ...material,
      updateDate: material.updateDate?.toJSON() ?? null,
      createDate: material.createDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restMaterial: RestMaterial): IMaterial {
    return {
      ...restMaterial,
      updateDate: restMaterial.updateDate ? dayjs(restMaterial.updateDate) : undefined,
      createDate: restMaterial.createDate ? dayjs(restMaterial.createDate) : undefined,
    };
  }
